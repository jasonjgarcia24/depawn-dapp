...
const getTransferibles = (recipient) => {
    return [
        {
...
describe("1-0 :: Loan Life Cycle", function () {
    let provider, borrower, lender, transferibles;

    beforeEach(async () => {
        provider = new ethers.providers.Web3Provider(hre.network.provider);
        [borrower, lender, ..._] = await hre.ethers.getSigners();
        transferibles = getTransferibles(borrower);
    });

    it("1-0-00 :: Impersonate Original NFT owners accounts. Transfer NFTs to hardhat accounts.", async function () {
        for (let tr of transferibles) {
            const owner = await hre.ethers.getSigners(tr.ownerAddress);
            console.log(owner)
            ...
            nftContract = new hre.ethers.Contract(tr.nft, tr.abi, owner);

            let tx = await nftContract["safeTransferFrom(address,address,uint256)"](
